...
describe('ApiClient', () => {
    ...
    it('normalizes Problem Details and network failures consistently', async () => {
        const problemClient = new ApiClient({
            fetcher: vi.fn(async () => jsonResponse(409, {
                title: 'Conflict',
                status: 409,
        ...
        await expect(problemClient.getPublicPolls()).rejects.toMatchObject({
            frontend: {
                kind: 'problem',
                status: 409,
                code: 'poll_state_conflict',
    ...
    it('clears protected query data and remembers the requested route after admin 401', async () => {
        ...
        const client = new ApiClient({
            fetcher: vi.fn(async () => jsonResponse(401, {
                status: 401,
                code: 'unauthorized'
            }, 'application/problem+json')),
    ...
    it('uses the dedicated endpoints for every admin poll lifecycle action', async () => {
        const poll = {
            ...
            endsAt: null,
            totalVotes: 0,
            templateGroup: {id: 'g_v1_group', name: 'Group', description: ''},
