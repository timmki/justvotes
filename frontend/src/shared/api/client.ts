...
import { ApiError, networkError, problemError } from './errors';
...
export class ApiClient {
  ...
  constructor(options: { fetcher?: Fetcher; onUnauthorized?: UnauthorizedHandler; onAuthenticated?: AuthenticatedHandler; onSessionEnded?: SessionEndedHandler } = {}) {
  ...
  async request<T>(path: string, options: RequestOptions = {}): Promise<T> {
    ...
    return this.send<T>(path, { ...options, method, headers, body }, path.startsWith('/admin/') && path !== '/admin/login');
  }

  getCsrf() { return this.refreshCsrf(); }
  ...
  getIdentity() { return this.request<CurrentIdentity>('/identity'); }
  changeIdentity(identity: Identity) { return this.request<void>('/identity', { method: 'POST', body: identity }); }
  getPublicPolls() { return this.request<Poll[]>('/polls'); }
  getPoll(pollId: string) { return this.request<Poll>(`/polls/${encodeURIComponent(pollId)}`); }
  getPollResults(pollId: string) { return this.request<PollResults>(`/polls/${encodeURIComponent(pollId)}/results`); }
  getPollAudit(pollId: string) { return this.request<AuditEntry[]>(`/polls/${encodeURIComponent(pollId)}/audit`); }
  ...
  private ensureCsrf() {
    ...
    if (!this.csrfRequest) {
      this.csrfRequest = this.send<CsrfToken>('/csrf', { method: 'GET' }, false).finally(() => { this.csrfRequest = null; });
    }
    return this.csrfRequest.then((token) => { this.csrf = token; return token; });
  }
...
async function readProblem(response: Response) {
  const text = await response.text();
  if (!text) return {};
  try { return JSON.parse(text) as Partial<components['schemas']['Problem']>; } catch { return {}; }
}
...
export const apiClient = new ApiClient({
  ...
  onSessionEnded: () => { clearProtectedQueries(); sessionCoordinator.consumeReturnRoute(); },
});
