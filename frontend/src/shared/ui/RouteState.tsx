...
export function RouteState({ status, onRetry = () => window.location.reload(), error, stale = false }: { status: StateStatus; onRetry?: () => void; error?: FrontendError; stale?: boolean }) {
  const { t } = useI18n();
  const content = {
    ...
    error: { title: error ? t(error.messageKey) : t('common.error'), text: error ? t(error.messageKey) : t('common.errorText'), icon: <ErrorIcon /> },
  }[status];
  ...
  return (
    <section className={`route-state ${status}`} aria-label={t('common.pageState')}>
      ...
      <div><h3>{content.title}</h3><p>{content.text}</p>{stale && <p className="stale-state" role="status">{t('common.refreshing')}</p>}{status === 'error' && <button className="text-button" type="button" onClick={onRetry}>{t('common.retry')}</button>}</div>
    </section>
  );
}
