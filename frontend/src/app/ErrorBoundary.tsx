...
export class AppErrorBoundary extends Component<{ children: ReactNode }, { hasError: boolean }> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) { console.error('Application rendering failed', error, info); }
...
function ErrorFallback() {
  ...
  return <div className="page-frame error-page"><p className="eyebrow">{errors.label}</p><h1>{errors.boundary}</h1><p>{errors.boundaryText}</p><a className="primary-button" href="/">{common.home}</a></div>;
}
