...
export function HomePage() {
    ...
    return <PageFrame eyebrow="JustVotes" title={t('common.home')} description={t('common.homeDescription')}><QueryState
        query={identityQuery}>{(identity) => <IdentityCard identity={identity.userID}/>}</QueryState>
        <nav className="primary-navigation" aria-label={t('common.mainNavigation')}><Link className="action-card"
                                                                                          to="/polls"><span><strong>{t('common.polls')}</strong><small>{t('common.publicArea')}</small></span><ChevronRightIcon/></Link><Link
            className="action-card secondary"
            to="/admin"><span><strong>{t('common.admin')}</strong><small>{t('common.adminArea')}</small></span><ChevronRightIcon/></Link>
        </nav>
    </PageFrame>;
}
...
function IdentityCard({identity}: { identity: string | null }) {
    ...
    return <section className="identity-card" aria-labelledby="identity-heading">
        ...
        {!editing ? <><h2 id="identity-heading">{identity ? <span aria-label={identity}
                                                                  title={identity}>{displayIdentity(identity)}</span> : t('common.identityNotSet')}</h2>
            <p>{t('common.identityNote')}</p>
            <button ref={editButtonRef} className="text-button" type="button"
                    onClick={startEditing}>{t('common.identityEdit')}</button>
        </> : <>
            <h2 id="identity-heading">{t('common.identityEdit')}</h2>
            {identity && <p className="confirmed-identity">{t('common.identity')}: <span
                title={identity}>{displayIdentity(identity)}</span></p>}
            <label htmlFor="identity-input">{t('common.identityInput')}</label>
            <input id="identity-input" value={draft} onChange={(event) => {
        ...
        {confirmOpen && <IdentityConfirmation onCancel={() => {
            setConfirmOpen(false);
            saveButtonRef.current?.focus();
        }} onConfirm={() => mutation.mutate(normalizeIdentity(draft))} pending={mutation.isPending}/>}
    </section>;
}
...
function IdentityConfirmation({onCancel, onConfirm, pending}: {
    ...
    return <div className="modal-backdrop">
        <div ref={dialogRef} className="identity-dialog" role="dialog" aria-modal="true"
             aria-labelledby="identity-dialog-heading" aria-describedby="identity-dialog-warning"><h3
            id="identity-dialog-heading">{t('common.identityChangeTitle')}</h3><p
            id="identity-dialog-warning">{t('common.identityChangeWarning')}</p>
            <div className="identity-actions">
...
export function NotFoundPage() {
    const {t} = useI18n();
    return <PageFrame eyebrow="404" title={t('errors.notFound')} description={t('errors.notFoundText')}><Link
        className="primary-button" to="/">{t('common.home')}</Link></PageFrame>;
}
