...
export function AdminPage() {
    ...
    if (sessionQuery.isError) return <AdminAccess mode="error" error={sessionQuery.error} onRetry={() => {
        void sessionQuery.refetch();
    }}/>;
    return <AdminShell section={section ?? 'votes'}/>;
}

function AdminAccess({mode, error, onRetry}: {
    mode: 'loading' | 'login' | 'error';
    error?: unknown;
    onRetry?: () => void
}) {
    const {t} = useI18n();
...
function AdminLoginForm() {
    ...
    return <form className="login-form" onSubmit={submit}>
        ...
        <input id="admin-username" name="username" autoComplete="username" required value={username}
               onChange={(event) => setUsername(event.target.value)}/>
        <label htmlFor="admin-password">{t('common.password')}</label>
        <input id="admin-password" name="password" type="password" autoComplete="current-password" required
               value={password} onChange={(event) => setPassword(event.target.value)}/>
        <button className="primary-button" type="submit"
                disabled={pending}>{pending ? t('common.signingIn') : error?.retryable ? t('common.retry') : t('common.signIn')}</button>
